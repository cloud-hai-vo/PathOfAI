# Path of AI — Complete Feature Specification
...
## DEFENSIVE ANALYSIS `[MVP | P1: Combat Simulator, Recovery Analysis]`
...
## OFFENSIVE ANALYSIS `[MVP | P1: Transfigured Gems, Critical Strike, Flask Optimization]`
...
## PASSIVE TREE ANALYSIS `[MVP: Node Scoring, Path Opt, Next Points | P1: Masteries, Keystones, Jewels | P2: Full Tree Modal, Cluster Jewels, Anoints]`
...
## ITEM ANALYSIS `[MVP: Ctrl+C Import, Mod Tiers, Open Affixes, The Forge | P1: Crafting Sim, Corruption, Enchantment, Sockets | P2: Watcher's Eye, Influence, Minion-specific]`
...
## INVENTORY INTELLIGENCE `[P1: Stash Scanner, Currency Tracker, Sellable Items | P2: Div Cards, Fragments]`
...
## MARKET INTELLIGENCE `[MVP: Live Prices, Upgrade Shopping | P1: Budget Path, Price Alerts | P2: Snipe Alerts, Scam Detection]`
...
## BUILD PROGRESSION SYSTEM `[MVP: Boss Readiness, Mapping Checklist | P1: Act Guide, League Mechanic, Content Readiness]`
...
## BLOOD PACT PANEL `[MVP]`

> **UI panel name:** ☠ Blood Pact (skull icon, blood-red)
> **What it is:** Build goals tracker + one-click AI action launcher — the pact you make with the Seer to complete your build.

### Sworn Goals Checklist
- Persistent checklist of player-defined or AI-suggested build objectives
- Each goal has a done/not-done toggle stored in `settings.json`
- Examples generated by the AI based on current build gaps:
  - "Corrupt main skill gem to 21/20"
  - "Upgrade belt to Headhunter"
  - "Cap chaos resistance"
  - "Unlock Uber Lab"
- Progress counter shown: "3 / 7 completed"
- Goals sync with Build Progression System (Boss Readiness, Mapping Checklist)
- AI repopulates goals when major upgrades are applied

### Blood Rituals (Quick AI Actions)
One-click action buttons that invoke the Seer to perform a specific optimization task immediately:

```
[🌳 Respec passive tree]       → Find spec saving 3+ points with same stats
[🔨 Channel benchcrafts]       → Auto-craft life/resist on open mod slots
[💎 Optimize gem links]        → Identify highest-DPS support gem swap
[📊 Full analysis]             → Run complete build audit, populate goals
```

- Each button triggers the corresponding Seer query via `invoke('ask_seer', ...)`
- Results displayed inline below the button
- "Invoke →" CTA label reinforces the PoE flavor
- Rituals are prioritized by current build state (e.g., only show benchcraft if open slots exist)

---

## DARK PATH PANEL `[MVP]`

> **UI panel name:** 🗡 Dark Path (dagger icon, chaos-purple)
> **What it is:** Build identity display + evolution path chooser — which dark path will you walk to complete your exile?

### Build Identity Card
- Displays current build archetype at a glance:
  - Build name (e.g., "RF Inquisitor")
  - Flavor tags (Fire DoT · High Regen · Block · Shield · Armour)
  - Tier rating badge: S-TIER / A-TIER / B-TIER based on poe.ninja meta score
- Derived from PoB import or OAuth character data + Seer classification

### Choose Your Dark Path (Evolution Paths)
AI-generated progression paths showing distinct directions the build can evolve. Each path has a trade-off profile:

```
🛡 The Immortal    — Max survivability, facetank everything
   • Key upgrades: Aegis Aurora, Melding of the Flesh
   • Cost: ~40-60d | DPS: -5% | Survivability: massive increase

🔥 The Inferno     — Max damage, melt bosses
   • Key upgrades: Mageblood, Headhunter or double-influenced chest
   • Cost: ~80-120d | DPS: +35% | Survivability: -10%

⚡ The Ascendant   — Balanced hybrid (default recommendation)
   • Key upgrades: Watcher's Eye, cluster jewels
   • Cost: ~20-30d | DPS: +15% | Survivability: +10%
```

- Paths are dynamically generated by the Seer based on current gear + passive tree
- Selecting a path sets the active goal direction used by the Sworn Goals checklist
- Path selection stored per-character in `settings.json`
- Paths are regenerated when the build changes significantly (new items, passive respec)
- Links to full Build Progression System (Act-by-Act Guide, Mapping Checklist) for path details

---

## MAP MOD DANGER SYSTEM `[MVP]`
...
## MULTI-CHARACTER MANAGEMENT `[P1: Dashboard, Switch, Share Codes | P2: Cross-Char Optimizer, Party Play]`
...
## AI-POWERED FEATURES `[MVP: Seer local engine | P1: Cloud providers (Claude/GPT) | P2: Build Concept Generator, Patch Analysis]`
...
## AUTOMATION & QUALITY OF LIFE `[P1: Hotkeys, Build Export | P2: Loot Filter, Trade Companion, Overlay]`
...
## DATA & ANALYTICS `[P1: Personal Stats, Map Run Tracker | P2: Economy Dashboard, Wealth History]`
...
## COMMUNITY FEATURES `[P1: Bug Report | P2: Build Sharing | IDEA: TFT Integration, Guide Integration]`
